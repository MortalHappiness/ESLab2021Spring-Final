...
#include "ISM43362Interface.h"
ISM43362Interface wifig(MBED_CONF_APP_WIFI_SPI_MOSI, MBED_CONF_APP_WIFI_SPI_MISO, MBED_CONF_APP_WIFI_SPI_SCLK, MBED_CONF_APP_WIFI_SPI_NSS, MBED_CONF_APP_WIFI_RESET, MBED_CONF_APP_WIFI_DATAREADY, MBED_CONF_APP_WIFI_WAKEUP, false);;
...
{
public:
    Wifi(Sensor *sensor, events::EventQueue &event_queue, ISM43362Interface* wifi)  : _sensor(sensor), _event_queue(event_queue), _wifi(wifi), _led1(LED1, 1)
    {
        // _wifi();
        connect();
    }
    ...
    void connect()
    {
        ...
        printf("\nConnecting to %s...\n", "Jeff0937");
        int ret = _wifi->connect("Jeff0937", "0937988311", NSAPI_SECURITY_WPA_WPA2);
        if (ret != 0)
        {
            printf("\nConnection error: %d\n", ret);
            return;
